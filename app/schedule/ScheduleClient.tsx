...
export default function ScheduleClient({
  ...
  async function handleConfirm() {
    if (!meetingType || !selectedDate || !selectedTime || submitting) return;
    setSubmitting(true);
    setSubmitError(null);
...
function Step2({
  ...
  /** "yyyy-mm-dd" strings admin has blocked. */
  blockedSet: Set<string>;
}) {
  const year = currentMonth.getFullYear();
  const month = currentMonth.getMonth();
  // Admin-blocked days take priority over the deterministic decorative
  // "booked" set the demo calendar uses for non-blocked dates.
  const blockedDayNumbers = useMemo(() => {
    const set = new Set<number>();
    for (const dayStr of blockedSet) {
      const m = dayStr.match(/^(\d{4})-(\d{2})-(\d{2})$/);
      if (!m) continue;
      const y = Number(m[1]);
      const mo = Number(m[2]) - 1;
      const d = Number(m[3]);
      if (y === year && mo === month) set.add(d);
    }
    return set;
  }, [blockedSet, year, month]);
  const bookedDates = useMemo(() => {
    const fake = getBookedDates(year, month);
    // Union of deterministic fake bookings + real admin blocks
    for (const d of blockedDayNumbers) fake.add(d);
    return fake;
  }, [year, month, blockedDayNumbers]);
